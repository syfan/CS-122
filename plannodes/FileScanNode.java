...
public class FileScanNode extends SelectNode {
    ...
    public void prepare() {
        ...
        // Initialize the cost
        cost = new PlanCost(tableStats.numTuples, tableStats.avgTupleSize, tableStats.numTuples, tableStats.numDataPages, 1);
        // If there is a predicate, use it to update the tuples
        if (predicate != null) {
            cost.numTuples *= SelectivityEstimator.estimateSelectivity(predicate, schema, tableStats.getAllColumnStats());
            // estimate cost of predicate by creating processor
            ExpressionCostCalculator processor = new ExpressionCostCalculator();
            predicate.traverse(processor);
            float temp = processor.getCost();
            temp *= cost.numTuples;
            cost.cpuCost += temp;
        }
