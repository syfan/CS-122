...
public abstract class ThetaJoinNode extends PlanNode {
    ...
    /**
     * The type of the join operation to perform.  Note that not all join
     * implementations can necessarily implement all possible join types.
     */
    ...
    /**
     * True if the output schema of this node is swapped.  If this flag is
     * true then tuples from the left subplan will be on the right of joined
     * results and so forth, but if the flag is false then the tuples from the
     * left subplan will be on the left of the joined results.  The schema and
     * the statistics values reflect the state of this flag, after
