...
public class SimpleFilterNode extends SelectNode {
    ...
    public void prepare() {
        ...
        // Initialize the cost to the left child
        PlanCost childCost = leftChild.getCost();
        cost = new PlanCost(childCost);
        // Increase cpuCost by number of tuples as each must be checked by SELECT
        cost.cpuCost += childCost.numTuples;
        // If there is a predicate, use it to estimate the new number of tuples
        if (predicate != null) {
