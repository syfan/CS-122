...
public class FunctionCall extends Expression {
    ...
    /**
     * Resolve this function-call against the function directory so that the
    ...
    /**
     * Returns the implementation of the function, or {@code null} if the
     * implementation has not yet been resolved via the {@link #resolve}
     * method.
     *
    ...
    public ColumnInfo getColumnInfo(Schema schema) throws SchemaNameException {
        if (function == null) {
            throw new IllegalStateException(String.format(
                "Function \"%s\" is not yet resolved", funcName));
        }

        if (!(function instanceof ScalarFunction)) {
            throw new ExpressionException(
                "This operation only works with subclasses of ScalarFunction");
        }

        // Generate a column-info object with the type indicated by the
        // function object, and with a column-name generated from the
        // expression.
        ScalarFunction sf = (ScalarFunction) function;
        ColumnType colType = sf.getReturnType(args, schema);
