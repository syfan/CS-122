...
public class TupleLiteral implements Tuple, Serializable {
    ...
    public TupleLiteral(Object... inputs) {
        if (inputs.length > 0 && inputs[0] instanceof Tuple) {
            throw new IllegalArgumentException(
                "Use TupleLiteral.fromTuple() to copy a Tuple object");
        }

        values = new ArrayList<>(inputs.length);
    ...
    public static TupleLiteral fromTuple(Tuple tuple) {
        TupleLiteral copy = new TupleLiteral();
        copy.appendTuple(tuple);
        return copy;
    }
