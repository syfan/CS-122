...
public abstract class PageTuple implements Tuple {
    ...
    private void setNullColumnValue(int iCol) {
        ...
        if (!isNullValue(iCol)) {
            ...
            int length = getColumnValueSize(schema.getColumnInfo(iCol).getType(), valueOffsets[iCol]);
            deleteTupleDataRange(valueOffsets[iCol],
                    getColumnValueSize(schema.getColumnInfo(iCol).getType(), valueOffsets[iCol]));
            pageOffset += length;
            computeValueOffsets();
    ...
    private void setNonNullColumnValue(int iCol, Object value) {
        ...
        if (isNullValue(iCol)) {
            ...
            for (int i = iCol - 1; i >= 0; i--) {
                if (!isNullValue(i)) {
                    offset = valueOffsets[i] + getColumnValueSize(schema.getColumnInfo(i).getType(), valueOffsets[i]);
                    break;
                }
            }
            if (schema.getColumnInfo(iCol).getType().getBaseType() == SQLDataType.VARCHAR) {
                String strValue = TypeConverter.getStringValue(value);
                length = strValue.length() + 2;
                insertTupleDataRange(offset, length);
                pageOffset -= length;
            } else {
                length = getColumnValueSize(schema.getColumnInfo(iCol).getType(), valueOffsets[iCol]);
                insertTupleDataRange(offset, length);
                pageOffset -= length;
            }
            writeNonNullValue(dbPage, offset - length, schema.getColumnInfo(iCol).getType(), value);
        }
        else {
            ...
            if (schema.getColumnInfo(iCol).getType().getBaseType() == SQLDataType.VARCHAR) {
                String strValue = TypeConverter.getStringValue(value);
                length = strValue.length() + 2;
                int old_length = getColumnValueSize(schema.getColumnInfo(iCol).getType(), valueOffsets[iCol]);
                deleteTupleDataRange(valueOffsets[iCol], old_length);
                pageOffset += old_length;
                insertTupleDataRange(offset + old_length, length);
                pageOffset -= length;
                offset -= length;
                offset += old_length;
            }
            writeNonNullValue(dbPage, offset, schema.getColumnInfo(iCol).getType(), value);
