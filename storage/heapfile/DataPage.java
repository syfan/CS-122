...
public class DataPage {
    ...
    public static void deleteTuple(DBPage dbPage, int slot) {
        ...
        deleteTupleDataRange(dbPage, off, tupleLength);
        setSlotValue(dbPage, slot, EMPTY_SLOT);

        // check if current slot is at the end of the header
        if (slot <= numSlots) {
            int iterSlot = slot;
            while (iterSlot < numSlots) {
                    if (getSlotValue(dbPage, iterSlot) != EMPTY_SLOT) {
                        return;
                    }
                    iterSlot += 1;
            }
            int newNumSlots = numSlots - (numSlots - slot);
            setNumSlots(dbPage, newNumSlots);
        }
