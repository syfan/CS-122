...
page_scan:  // So we can break out of the outer loop from inside the inner loop.
        while (true) {
            ...
            // dbPage no longer needs to be pinned
            dbPage.unpin();
    ...
    public Tuple addTuple(Tuple tup) {
        ...
        while (true) {
            ...
            // page no longer needed if we got here
            dbPage.unpin();
    ...
    @Override
    public void analyze() {
        // store the number of tuples in the table file, average size of the tuples,
        // and the number of pages - 1 to exclude the header page.
        int numTuples = 0;
        float avgTupleSize = 0;
        int numPages = dbFile.getNumPages() - 1;

        // keep track os stats for each column
        ArrayList<ColumnStats> colStats = new ArrayList<>();
        // create ColumnStatsCollectors for our list of them
        int numCols = schema.numColumns();
        ColumnStatsCollector[] colStatsCollectors = new ColumnStatsCollector[numCols];

        // get the types needed for each ColumnStatsCollector
        for (int i = 0; i < numCols; i++) {
            colStatsCollectors[i] = new ColumnStatsCollector(schema.getColumnInfo(i).getType().getBaseType());
        }

        // variables for use when iterating through data pages
        DBPage currPage;
        HeapFilePageTuple currPageTuple;
        int currSlotValue;

        // begin page iteration at index 1 to skip header
        for (int i = 1; i <= numPages; i++){
            currPage = storageManager.loadDBPage(dbFile, i);
            // sum up the sizes of tuples, to later be divided by number of tuples
            avgTupleSize += DataPage.getTupleDataEnd(currPage) - DataPage.getTupleDataStart(currPage);

            int numSlots = DataPage.getNumSlots(currPage);
            for (int j = 0; j < numSlots; j++) {
                currSlotValue = DataPage.getSlotValue(currPage, j);
                if (currSlotValue != DataPage.EMPTY_SLOT) {
                    numTuples += 1;
                    // create a HeapFilePageTuple for our current page's tuple
                    currPageTuple = new HeapFilePageTuple(schema, currPage, j, currSlotValue);
                    for (int k = 0; k < numCols; k++) {
                        // and ensure that we collect the necessary data from it
                        colStatsCollectors[k].addValue(currPageTuple.getColumnValue(k));
                    }
                }
            }
        }
        // dividing sum of all tuples by number of tuples to get average
        avgTupleSize = avgTupleSize / numTuples;
        // ensure we have the stats for each column
        for (ColumnStatsCollector colStatsCollector : colStatsCollectors) {
            colStats.add(colStatsCollector.getColumnStats());
        }

        TableStats tableStats = new TableStats(numPages, numTuples, avgTupleSize, colStats);
        stats = tableStats;
