...
import edu.caltech.nanodb.storage.btreefile.DataPage;
...
public class WALManager {
    ...
    private void performRedo(RecoveryInfo recoveryInfo) {
        ...
        while (currLSN.compareTo(recoveryInfo.nextLSN) < 0) {
            ...
            switch (type) {
                /* If we are at a start state, add the transaction to the list of
                ...
                // If we are at either update page, than apply the redo
                case UPDATE_PAGE:
                case UPDATE_PAGE_REDO_ONLY:
                    if (!recoveryInfo.isTxnComplete(transactionID)) {
                        int fileNo = walReader.readUnsignedShort();
                        ...
                        int num_segments = walReader.readUnsignedShort();
                        applyRedo(type, walReader, dbPage, num_segments);
                        recoveryInfo.updateInfo(transactionID, currLSN);
                        walReader.movePosition(5);
                    }
                    break;

                // When a transaction completes remove it from the incomplete list
                case ABORT_TXN:
                case COMMIT_TXN:
                ...
                default:
                    throw new WALFileException(
                            "Encountered bad WAL record type " + type + " at LSN "
    ...
    private void performUndo(RecoveryInfo recoveryInfo) {
        ...
        while (recoveryInfo.hasIncompleteTxns()) {
            ...
            switch (type) {
                // When we reach the start we are done undoing the transaction
                case START_TXN:
                    break;

                // If we are at an update page, undo the results
                // Write a redo only update to show thi
                case UPDATE_PAGE:
                    int fileNo = walReader.readUnsignedShort();
                    int offset = walReader.readInt();
                    LogSequenceNumber prevlsn = new LogSequenceNumber(fileNo, offset);
                    ...
                    dbPage.setPageLSN(currLSN);
                    int num_segments = walReader.readUnsignedShort();
                    byte[] change = applyUndoAndGenRedoOnlyData(walReader, dbPage, num_segments);
                    writeRedoOnlyUpdatePageRecord(transactionID, prevlsn, dbPage, num_segments,
                            change);
                    currLSN = prevlsn;
                    break;

                // Ignore redo only updates
                case UPDATE_PAGE_REDO_ONLY:
                    break;
    ...
    public void rollbackTransaction() {
        ...
        logger.info("Rolling back transaction %d.  Last LSN = %s",
            transactionID, lsn);
        ...
        while (true) {
            ...
            logger.debug("Undoing WAL record at %s.  Type = %s, TxnID = %d",
                lsn, type, transactionID);


            // Break when we have reached the start of the transaction
            if (type == WALRecordType.START_TXN) {
                break;
            }
            // If we are at an update page, undo the change and write redo only record
                ...
                int num_segments = walReader.readUnsignedShort();
                byte[] change = applyUndoAndGenRedoOnlyData(walReader, dbPage, num_segments);
                writeRedoOnlyUpdatePageRecord(dbPage, num_segments, change);
                // Switch to the previous lsn in the transaction.
                lsn = prevlsn;
            }
            else {
                throw new WALFileException("WAL Record Type isn't Start or Update_Page");
            }
        }
