 ...
 *       <li>For each column:
 *         <ul>
 *           <li>Zero-based index of NOT-NULL Column (unsigned byte)</li>
 *         </ul>
 *       </li>
 *     </ul>
 ...
 *   <dd>
 *     <ul>
 *       <li>Total Number of Candidate + Foreign Keys (unsigned byte)</li>
 *       <li>For each candidate key (which includes primary key, if any):
 *         <ul>
 ...
 *           <li>For each column:
 *             <ul>
 *               <li>Zero-based index of Column in Key (unsigned byte)</li>
 *             </ul>
 *           </li>
 *         </ul>
 *       </li>
 ...
 *   </dd>
 *
 *   <dt>Referencing Tables:</dt>
 *
 *   <dd>
 *     <ul>
 *       <li>Total Number of Referencing Tables (unsigned byte)</li>
 *       <li>For each referencing table:
 *         <ul>
 ...
 *           <li>For each column:
 *             <ul>
 *               <li>Zero-based index of Column in Index (unsigned byte)</li>
 *             </ul>
 *           </li>
...
public class SchemaWriter {
    ...
    public void writeSchema(Schema schema, PageWriter pgWriter) {
        ...
        // Write all candidate/foreign key constraints:
        writeKeyConstraints(schema, pgWriter);

        // Write the tables that reference this table:
        writeReferencingTables(schema, pgWriter);
    ...
    protected void writeKeyConstraints(Schema schema,
        ...
        for (KeyColumnRefs ck : schema.getCandidateKeys())
            writeKey(pgWriter, ck.getConstraintType(), ck);
    ...
    protected void writeReferencingTables(Schema schema, PageWriter pgWriter) {
        ...
        // Count how many tables we have to write.
        Set<String> refTables = schema.getReferencingTables();

        logger.debug("Recording " + refTables.size() + " referencing tables");
        pgWriter.writeByte(refTables.size());
        for (String refTable : refTables) {
            pgWriter.writeVarString255(refTable);
        }

        if (logger.isDebugEnabled()) {
            int size = pgWriter.getPosition() - startPosition;
            logger.debug("Referencing tables occupy " + size + " bytes.");
        }
    ...
    public Schema readSchema(PageReader pgReader) {
        ...
        // Read the tables that reference this table:
        readReferencingTables(pgReader, schema);
    ...
    protected void readColumnInfos(PageReader pgReader, Schema schema) {
        ...
        ArrayList<String> tableNames = new ArrayList<>(numTables);
        ...
        if (numCols == 0)
            throw new DataFormatException("Table must have at least one column.");
        ...
        for (int iCol = 0; iCol < numCols; iCol++) {
            ...
            if (baseType == null) {
                throw new DataFormatException("Unrecognized SQL type " +
                    sqlTypeID + " for column " + iCol);
            }
            ...
            if (colName.length() == 0) {
                throw new DataFormatException("Name of column " + iCol +
                    " is unspecified.");
            }
            ...
            for (int iCh = 0; iCh < colName.length(); iCh++) {
                ...
                if (iCh == 0 && !(Character.isLetter(ch) || ch == '_' || ch == '#') ||
                    ...
                    throw new DataFormatException(String.format(
                        "Name of column %d \"%s\" has an invalid character at index %d.",
                        iCol, colName, iCh));
                }
            }
        ...
        for (int i = 0; i < numConstraints; i++) {
            ...
            if (cType == null) {
                throw new DataFormatException(
                    "Unrecognized constraint-type value " + cTypeID);
            }
            ...
            switch (cType) {
                case PRIMARY_KEY:
                case UNIQUE:
                    key = readKey(pgReader, cTypeID, cType);
    ...
    protected KeyColumnRefs readKey(PageReader pgReader, int typeID,
        ...
        return new KeyColumnRefs(keyCols, constraintName, type);
    }
    ...
    protected IndexColumnRefs readIndex(PageReader pgReader) {
        ...
        return new IndexColumnRefs(indexName, idxCols);
    }
}
