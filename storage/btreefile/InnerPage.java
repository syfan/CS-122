...
public class InnerPage implements DataPage {
    ...
    public TupleLiteral movePointersLeft(InnerPage leftSibling, int count,
        ...
        // Get the length to move
        int len = pointerOffsets[count - 1] + 2 - OFFSET_FIRST_POINTER;

        // Initialize the leftPage
        DBPage leftPage =  leftSibling.getDBPage();

        // Get the newParentLey
        TupleLiteral newParentKey = null;
        if (count != numPointers) {
            newParentKey =  TupleLiteral.fromTuple(keys[count - 1]);
        }
        ...
        // Write the data in the left page
        leftPage.write(leftSibling.getUsedSpace() + parentKeyLen,
                dbPage.getPageData(), OFFSET_FIRST_POINTER, len);

        // Move the data in the current page to the front of the page
        if (count != numPointers) {
            dbPage.moveDataRange(pointerOffsets[count],
                    OFFSET_FIRST_POINTER, endOffset - pointerOffsets[count]);
        }

        // Update the number of pointers in the two pages
...
*/
    ...
    public TupleLiteral movePointersRight(InnerPage rightSibling, int count,
        ...
        // Initialize the right page from the rightSibling
        DBPage rightPage =  rightSibling.getDBPage();

        // Get the new parent key
        TupleLiteral newParentKey = null;
        if (numPointers > count) {
            newParentKey =  TupleLiteral.fromTuple(keys[numPointers - count - 1]);
        }

        // Move the data to the right in the right page to make room for new tup;es
        rightPage.moveDataRange(OFFSET_FIRST_POINTER,
                OFFSET_FIRST_POINTER + len + parentKeyLen,
                rightSibling.getSpaceUsedByEntries());

        // Write the data into the right page
        rightPage.write(OFFSET_FIRST_POINTER,
                dbPage.getPageData(), startOffset, len);

        // Move Parent right
        if (parentKey != null) {
            PageTuple.storeTuple(rightPage,
                    OFFSET_FIRST_POINTER + len, schema,
                    parentKey);
        }

        // Update the number of tuples in the two pages
        dbPage.writeShort(OFFSET_NUM_POINTERS, numPointers - count);
