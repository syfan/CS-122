...
public class LeafPageOperations {
    ...
    private BTreeFilePageTuple splitLeafAndAddTuple(LeafPage leaf,
        ...
        // Save the old next page number, and add a new leaf in the middle
        int nextLeafPageNo = leaf.getNextPageNo();
        leaf.setNextPageNo(newLeaf.getPageNo());
        newLeaf.setNextPageNo(nextLeafPageNo);

        // Get the number of tuples to split on
        int numTuples = leaf.getNumTuples();
        int splitindex = numTuples / 2;

        // Move half the tuples to the right
        leaf.moveTuplesRight(newLeaf, splitindex);

        BTreeFilePageTuple returnPageTuple = addTupleToLeafPair(leaf, newLeaf, tuple);

        // If there isn't a parent, create a new root node
        if (pagePath.size() == 1) {
            DBPage dbpRoot = fileOps.getNewDataPage();
            InnerPage root = InnerPage.init(dbpRoot, tupleFile.getSchema(),
                    leaf.getPageNo(), newLeaf.getTuple(0), newLeaf.getPageNo());

            DBPage dbpHeader = storageManager.loadDBPage(tupleFile.getDBFile(),
                    0);
            assert (leaf.getPageNo() == HeaderPage.getRootPageNo(dbpHeader));
            HeaderPage.setRootPageNo(dbpHeader, root.getPageNo());
        }
        else {
            // Get the parent page number and load that page
            pagePath.remove(pagePath.size() - 1);
            int parentPageNo = pagePath.get(pagePath.size() - 1);
            InnerPage parentPage = innerPageOps.loadPage(parentPageNo);

            // Upgrade the first tuple of the new leaf to the parent node's table
            Tuple upgradedTuple = newLeaf.getTuple(0);
            innerPageOps.addTuple(parentPage, pagePath, leaf.getPageNo(),
                    upgradedTuple, newLeaf.getPageNo());
        }

        return returnPageTuple;
    }
