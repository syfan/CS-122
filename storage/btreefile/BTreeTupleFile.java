...
public class BTreeTupleFile implements SequentialTupleFile {
    ...
    private LeafPage navigateToLeafPage(Tuple searchKey,
        ...
        // Loop while the page is not a leaf
        while (pageType != BTREE_LEAF_PAGE) {
            // Create a new innerPage from the dbPage
            InnerPage innerPage = new InnerPage(dbPage, schema);
            // Calculate the number of pointers in the page
            int m = innerPage.getNumPointers();
            int i = 1;
            // Loop over the pointers i
            while (true) {
                if (TupleComparator.comparePartialTuples(searchKey,
                        innerPage.getKey(i - 1),
                        TupleComparator.CompareMode.SHORTER_IS_LESS) < 0) {
                    // Follow the i - 1 pointer.
                    int pointerPageNo = innerPage.getPointer(i - 1);
                    dbPage = storageManager.loadDBPage(dbFile, pointerPageNo);
                    pageType = dbPage.readByte(0);

                    if (pagePath != null)
                        pagePath.add(pointerPageNo);
                    break;
                } else if (TupleComparator.comparePartialTuples(searchKey,
                        innerPage.getKey(i - 1),
                        TupleComparator.CompareMode.SHORTER_IS_LESS) == 0) {
                    // Follow the i pointer
                    int pointerPageNo = innerPage.getPointer(i);
                    dbPage = storageManager.loadDBPage(dbFile, pointerPageNo);
                    pageType = dbPage.readByte(0);
                    ...
                    // We ran out of pointers, return the last one
                    int pointerPageNo = innerPage.getPointer(innerPage.getNumKeys());
                    dbPage = storageManager.loadDBPage(dbFile, pointerPageNo);
                    pageType = dbPage.readByte(0);
        ...
        LeafPage leafPage = new LeafPage(dbPage, schema);
        return leafPage;
    }
