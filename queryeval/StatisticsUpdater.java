...
public class StatisticsUpdater {
     ...
     * "COLUMN = VALUE".
     *
     * @param comp2 The comparison operation to consider
     ...
     */
    private static void updateCompareStats(CompareOperator comp2,
                                           Schema schema,
        ...
        if (left instanceof ColumnValue && right instanceof LiteralValue) {
            ...
            switch (comp.getType()) {
                ...
                case EQUALS: {
                    float prob = SelectivityEstimator.estimateSelectivity(right, schema, stats);
                    stat.setMinValue(value);
                    stat.setMaxValue(value);
                    stat.setNumUniqueValues(1);
                    stat.setNumNullValues(0);

                    // loop through column stats to update (except target column)
                    for (int i = 0; i < stats.size(); i++){
                        if (i == colIdx) {
                            continue;
                        }
                        stat = stats.get(i);
                        // approximate new amount of null values in column
                        int oldNumNull = stat.getNumNullValues();
                        int newNumNull = (int)((float) oldNumNull * prob);
                        stat.setNumNullValues(newNumNull);
                    }
                    break;
                }
                case NOT_EQUALS: {
                ...
                case GREATER_THAN: {
                    ...
                    float prob = SelectivityEstimator.estimateSelectivity(right, schema, stats);
                    int oldNumUnique = stat.getNumUniqueValues();
                    int newNumUnique = (int)((float) oldNumUnique * prob);

                    // modify stats of current target column
                    stat.setMinValue(value);
                    stat.setNumUniqueValues(newNumUnique);
                    stat.setNumNullValues(0);
