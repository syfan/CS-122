...
public class TransactionManager implements BufferManagerObserver {
    ...
    public void beforeWriteDirtyPages(List<DBPage> pages) {
        // For each dbPage, if it is a type that has a LSN, forceWAL that LSN
        for (DBPage page : pages) {
            if (page.getDBFile().getType() != DBFileType.WRITE_AHEAD_LOG_FILE &&
                ...
                if (page.getPageLSN() == null) {
                    throw new IllegalStateException("pageLSN should not be null");
                }
    ...
    public void forceWAL(LogSequenceNumber lsn) {
        ...
        // is the argument lsn is <= the txnStateNextLSN, we have a no-op case.
        if (lsn.compareTo(txnStateNextLSN) <= 0) {
            return;
        }

        // keeping track of the current current DBFile and buffer Manager
        DBFile currFile = null;
        BufferManager bufferManager = storageManager.getBufferManager();

        // obtaining info from lsn argument and txnStateNextLSN
        ...
        // convenient  for writeDBFile calls later
        int minPageNo;
        int maxPageNo;

        /*
         bufferManager.getFile() is called to see if a file is open. NanoDB
         syncs files when it closes them, so syncing should only be required if
         a file is currently open (as stated in the assignment). The call to
         getFile() returns null if the file is not open.
          */

        if (oldLsnFileNo == lsnFileNo) {
            // in this case the file that currFile is in is the same as
            // the file that txnStateNextLSN is on, so it is not necessary to
            // write to any pages before txnStateNextLSN.
            currFile = bufferManager.getFile(WALManager.getWALFileName(oldLsnFileNo));
            if (currFile != null) {
                int currPageSize = currFile.getPageSize();
                minPageNo = oldLsnFileOffset / currPageSize;
                maxPageNo = (lsnFileOffset + lsnRecordSize) / currPageSize;
                bufferManager.writeDBFile(currFile, minPageNo, maxPageNo, true);
            }

        }
        else {
            // since currDBFile is not the file that lsn  or txnStateNextLSN on, every
            // page in each file must be written.
            for (int currLsnNo = oldLsnFileNo; currLsnNo <= lsnFileNo; currLsnNo ++) {
                currFile = bufferManager.getFile(WALManager.getWALFileName(currLsnNo));
                if (currFile != null) {
                    int currPageSize = currFile.getPageSize();

                    if (currLsnNo == oldLsnFileNo) {
                        minPageNo = oldLsnFileOffset / currPageSize;
                        maxPageNo = Integer.MAX_VALUE;
                        bufferManager.writeDBFile(currFile, minPageNo, maxPageNo, true);
                    }
                    // if the file that currFile is on is the same as the one that lsn is, then
                    // we write pages up to the page
                    else if (currLsnNo == lsnFileNo) {
                        maxPageNo = (lsnFileOffset + lsnRecordSize) / currPageSize;
                        bufferManager.writeDBFile(currFile, 0, maxPageNo, true);
                    }
                    else {
                        bufferManager.writeDBFile(currFile, true);
                    }
                }
            }
        }
        // the new txnStateNextLsn value needs to be computed using the current one
        // plus the size of its record.
        int lastPosition = lsn.getFileOffset() + lsn.getRecordSize();
        txnStateNextLSN = WALManager.computeNextLSN(lsn.getLogFileNo(), lastPosition);
        // write and sync txnstate.dat to the disk
        storeTxnStateToFile();
    }
